    ...
    def __init__(self,mysetdir=None, debugflag = False,NUMFRXSEQ=2,rawflag=4):
        ...
        for ii in range(2):
            for sample_id in self.sample_id_list:
                ...
                shift_ = np.random.randint(5, 10)
                rawflag = 4

                # crop
                H, W = gt_full_size_image.shape[1:3]
                if np.random.randint(10) < 5:
                    gt_full_size_image = gt_full_size_image ** (2.2)
                ...
                if self.crop_size > 100:
                    ...
                    hh_beg = np.random.randint(self.crop_size, H - self.crop_size-self.crop_size-1)
                    ww_beg = np.random.randint(self.crop_size, W - self.crop_size-self.crop_size-1)
                    ...
                    shift_ = -self.crop_size//2
                    preb = gt_full_size_image[:, hh_beg:hh_beg+ self.crop_size,\
                                                     ww_beg+shift_:ww_beg + self.crop_size+shift_]
                    shift_ = self.crop_size // 2
                    prob = gt_full_size_image[:, hh_beg:hh_beg + self.crop_size, \
                           ww_beg + shift_:ww_beg + self.crop_size + shift_]

                gt_patch_2 = np.ascontiguousarray(np.array(gt_patch_2))
                raw_patch_1 = np.ascontiguousarray(np.array(rgb2raw( gt_patch_1) ) )
                raw_patch_2 = np.ascontiguousarray(np.array(rgb2raw(gt_patch_2)))
                preb = np.ascontiguousarray(np.array(rgb2raw( preb)))
                prob = np.ascontiguousarray(np.array(rgb2raw( prob)))

                self.gt_patchs.append(gt_patch_2)
                self.gt_patchs.append(raw_patch_1)
                self.gt_patchs.append(raw_patch_2)
                self.gt_patchs.append(preb)
                self.gt_patchs.append(prob)

    def __getitem__(self,index):
        gtp = self.gt_patchs[index * 5]
        inp1 = self.gt_patchs[index * 5+1]
        inp2 = self.gt_patchs[index * 5+2]
        inp3 = self.gt_patchs[index * 5+3]
        inp4 = self.gt_patchs[index * 5+4]

        gtp = torch.from_numpy(gtp).float()
        inp1 = torch.from_numpy(inp1).float()
        inp2 = torch.from_numpy(inp2).float()
        inp3 = torch.from_numpy(inp3).float()
        inp4 = torch.from_numpy(inp4).float()
        input_patch_torch = torch.cat([inp1, inp2, inp3, inp4],0)

        return input_patch_torch, gtp

    def __len__(self):
        return len(self.gt_patchs)//5
