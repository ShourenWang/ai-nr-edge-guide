...
class myDataset(torch_data.Dataset):
            ...
            for sample_id in self.sample_id_list:
                ...
                for aa in range(14):
                    ...
                    if self.crop_size > 100:
                        ...
                        hh_beg = np.random.randint(0, H - self.crop_size-14*step-1)
                        ww_beg = np.random.randint(0, W - self.crop_size-14*step-1)
                        gt_patch_1 = gt_full_size_image[:, hh_beg+shift_:hh_beg+shift_ + self.crop_size,\
                                                         ww_beg+shift_:ww_beg + self.crop_size+shift_]

                    if rawflag==4:
                        gt_patch_1 = np.concatenate((np.expand_dims(gt_patch_1[0, 0::2, 0::2],0),
    ...
    def __getitem__(self,index):
        input_patch = np.array( self.gt_patchs[14 * index:14 * index+14])

        input_patch_torch = torch.from_numpy(input_patch).float()
        input_patch_2_torch = torch.from_numpy(input_patch).float()

        return input_patch_torch, input_patch_2_torch

    def __len__(self):
        return  len(self.gt_patchs)//14
