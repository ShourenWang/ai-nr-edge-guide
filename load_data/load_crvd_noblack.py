...
def decode_data(data_name,trainflag,full_res):
	...
	W = 1920
	if trainflag:
		xx = np.random.randint(0, (W - hyper_cfg.image_width * 2 - 2) ) // 2 * 2
		yy = np.random.randint(0, (H - hyper_cfg.image_height * 2 - 2) ) // 2 * 2
	...
	if full_res:
		noise_map = np.zeros((2,1080//32*16,1920//2))
	else:
		noise_map = np.zeros((2,hyper_cfg.image_height,hyper_cfg.image_width))
