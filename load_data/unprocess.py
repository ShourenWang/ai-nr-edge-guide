...
def mosaic(image):
  ...
  out = torch.stack((red, green_red, green_blue, blue), dim=-1)# emvd 需要rggb
  out  = torch.reshape(out, (shape[0] // 1, shape[1] // 1, 4))
  out  = out.permute(2, 0, 1) # Re-Permute the tensor back to CxHxW format
  return out
...
def unprocess(image):
  ...
  size_out = (image.size()[0],image.size()[1]//3*4,image.size()[2]//1,image.size()[3]//1)
  outs = torch.zeros(size_out, dtype=image.dtype, device=image.device)
  ...
  for i in range(image.size()[0]):
    ...
    # temp = safe_invert_gains(temp, rgb_gain, red_gain, blue_gain)
