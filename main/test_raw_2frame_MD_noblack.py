...
from models.cvpr.raw_2frame_MD_v6 import FastDVDnet
...
# tensor = torch.rand(1, 4*5, 720//2, 1280//2)
# tensor2 = torch.rand(1, 1, 720//2, 1280//2)
# flops, params = profile(G , inputs=(tensor,tensor2,))
# print("FLOPs: ", flops/2**30 * 664.99/549.81)
# e
...
checkpoint = torch.load(cfg.model_save_root+'modelv6b1_noblack.pth')
...
def test_big_size_raw(input_data, denoiser,noise_map, patch_h=256, patch_w=256, patch_h_overlap=64, patch_w_overlap=64):
	...
	while (patch_h * h_index - patch_h_overlap * (h_index - 1)) < H:
		...
		while (patch_w * w_index - patch_w_overlap * (w_index - 1)) < W:
			...
			with torch.no_grad():
				_,_,output_patch = denoiser(test_patch, noise_map)
