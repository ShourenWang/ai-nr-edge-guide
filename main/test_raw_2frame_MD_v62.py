...
from models.nr_5d.fastdvd_ori import FastDVDnet
from models.nr_2d.emvd import MainDenoise as FastDVDnet
from models.cvpr.raw_2frame_MD_v62  import MainDenoise as FastDVDnet
...
# tensor = torch.rand(1, 4*5, 720, 1280)
# tensor2 = torch.rand(1, 1, 720, 1280)
# flops, params = profile(G , inputs=(tensor,))
# # stat(G, (4*5, 1080, 1920))
# print("FLOPs: ", flops/2**30/553.58 *664.99)
# e
G = nn.DataParallel(G, device_ids=device_ids).cuda()

# Optimizer
G_optimizer = optim.Adam(G.parameters(), lr=cfg.learning_rate)
checkpoint = torch.load(cfg.model_save_root+'modelv62_best.pth')
start_epoch = checkpoint['epoch']
G.load_state_dict(checkpoint['model'],True)
...
# G = FastDVDnet().cuda()
# tensor = (torch.rand(1, 3*5, 720//32*16, 1280//32*16).cuda(),\
# 		  torch.rand(1, 1, 720//32*16, 1280//32*16).cuda(), )
# flops = FlopCountAnalysis(G, tensor)
# print("FLOPs: ", flops.total()/2**30 * 22.16 /134.62 )
...
def postprocess(output):
	...
	output = (output*(2**12-1.0) - 240 ) / (2**12-1.0 - 240)
	output = np.clip(output, 0, 1)
	return output

def test_big_size_raw(input_data, denoiser,noise_map, patch_h=256, patch_w=256, patch_h_overlap=64, patch_w_overlap=64):
	H = input_data.shape[1]
	W = input_data.shape[2]
	#print(input_data.shape) #1,w,h,c
	test_result = np.zeros((input_data.shape[0], H, W, 4))
	noise_map = torch.from_numpy(np.array(noise_map)).float().cuda()
	...
	while (patch_h * h_index - patch_h_overlap * (h_index - 1)) < H:
		...
		while (patch_w * w_index - patch_w_overlap * (w_index - 1)) < W:
			...
			with torch.no_grad():
				_,_,_,_,output_patch = denoiser(test_patch, noise_map)
	...
	for scene_id in range(7,12):
		...
		for i in range(1, 7 + 1):
			...
			if i == 1:
				...
				# raw = cv2.imread(
				# 	'/home/data_beifen/crvd_nr/indoor_raw_noisy/scene{}/ISO{}/frame{}_noisy0.tiff'.format(scene_id, iso,
				# 																						  i-1),
				# 	-1)
				# pre = np.expand_dims(pack_rggb_raw(raw), axis=0)
			...
			noisy_level = b_list[noisy_level_ind]
			# noise_map = np.zeros((1,2,1080//32*16,1920//2))
			noise_map = np.zeros((1, 2, cfg.image_height, cfg.image_height))
			noise_map[0,0, :, :] = (np.sqrt(a_list[noisy_level_ind]) / (2 ** 12 - 1.0)) ** 2
			noise_map[0,1, :, :] = (np.sqrt(b_list[noisy_level_ind]) / (2 ** 12 - 1.0)) ** 2

			test_result = test_big_size_raw(input_data, G,noise_map, patch_h=cfg.image_height, patch_w=cfg.image_width, patch_h_overlap=cfg.image_height//4,
											patch_w_overlap=cfg.image_width//4)  #  c,h,w

			# test_result = test_big_size_raw(input_data, G, noise_map, patch_h=1080//32*16, patch_w=1920//2,
			# 								patch_h_overlap=cfg.image_height // 2,
			# 								patch_w_overlap=cfg.image_width // 4)  # c,h,w

			pre = test_result
			# # if iso==25600 and i==1 and scene_id==10:
			# temp  = np.clip( test_result[0,:,:,:].transpose(0,1,2),0,1)
			# # temp = np.clip(input_data[0, :, :, 4:], 0, 1)
			# rgb = temp[:,:,:-1]
			# rgb[:,:,2] = temp[:,:,3]
			# cv2.imwrite('./results/iso'+str(iso)+str(scene_id)+'.png',np.uint8(rgb*255)[:,:,::-1])
			...
			# rgb = test_gt1[:, :, :-1]
			# rgb[:, :, 2] = test_gt1[:, :, 3]
			# cv2.imwrite('./results/iso'+str(iso)+str(scene_id)+'_gt.png', np.uint8(rgb * 255)[:, :, ::-1])
