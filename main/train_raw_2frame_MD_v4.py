...
def train(**args):
    ...
    start_epoch = 0
    checkpoint = torch.load(cfg.model_save_root+'modelv6_noblack.pth')
    # start_epoch = checkpoint['epoch']
    G.load_state_dict(checkpoint['model'],False)
    ...
    for epoch in range(start_epoch, cfg.epochs*5):
        ...
        if epoch % 27 == 0 and epoch > 0:
            ...
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=cfg.batch_size,
                                                       ...
                                                       shuffle=True, pin_memory=True)
        # if epoch % 27 ==0:
        #     G_optimizer = optim.Adam(G.parameters(), lr=cfg.learning_rate*10)
        # else:
        #     G_optimizer = optim.Adam(G.parameters(), lr=cfg.learning_rate)
        # train
        d_loss = 0
        ...
        for iter, data in enumerate(train_loader):
            ...
            for tt in range(cfg.frame_num):
                ...
                gt_train_gray = torch.mean(gt_train, 1).unsqueeze(1)
                edgegt = gt_train_gray - blur(gt_train_gray).detach()
                edgeloss2 = msecriterion(in_edge, edgegt)
                content_loss = contentloss(x, gt_train)

                gen_loss = content_loss + edgeloss2
            ...
            if epoch % 10 == 0 and iter == 0:
                torch.save({
                    ...
                    cfg.model_save_root+'modelv6b1_noblack.pth')
            if iter == 7 and epoch % 2 ==0:
                ...
                if eval_psnr > best_psnr:
                    ...
                    torch.save({
                        ...
                        'optimizer': G_optimizer.state_dict(),
                        'best_psnr': best_psnr},
                        os.path.join(cfg.model_save_root, 'modelv6b1_noblack_best.pth'))
