...
from load_data.load_crvd_noblack import *
...
from utils.utils import BCEloss,loss_tv,loss_color,loss_wavelet
...
def train(**args):
    ...
    train_dataset = loadImgs(train_data_name_queue,True,False)
    ...
    vgg_model = VGG19_Extractor(output_layer_list=[1,2,3,4]).cuda()
    canny_operator = CannyDetector().cuda()
    cfa = Debayer5x5().cuda()
    blur = g_blur(3, 1, 1).cuda()
    ...
    #     {"params": G.module.temp1.parameters()},
    #     {"params": G.module.temp2.parameters(), "lr": 0.0002}, ],
    #     lr=cfg.learning_rate,  # 默认参数
    # )
    ...
    start_epoch = 0
    # checkpoint = torch.load(cfg.model_save_root+'modelv6b1_noblack_sigma_best.pth')
    ...
    for epoch in range(start_epoch, cfg.epochs*5):
        print('------------------------------------------------')
        print('full res Epoch                |   ', ('%06d' % epoch))
        ...
        if epoch < cfg.epochs*3:
            ...
            G_optimizer = optim.Adam(G.parameters(), lr=cfg.learning_rate/10.0)
        # train
        d_loss = 0
        g_loss = 0
        eval_psnr = 0
        ...
        for iter, data in enumerate(train_loader):
            ...
            for tt in range(cfg.frame_num):
                ...
                if tt == 0:
                    ...
                    # pre = img_trains[:,tt*4-4:tt*4,:,:]
                    inputn = torch.cat([pre, cur], 1)
                gamma, fusion_out, denoise_out, omega, x = G(inputn, noise_map)
                ...
                loss_ct = loss_color(G, ['ct.net1.weight', 'cti.net1.weight'])
                loss_ft = loss_wavelet(G)
                gen_loss = gen_loss + loss_ct + loss_ft
            ...
            if epoch % 10 == 0 and iter == 0:
                torch.save({
                    ...
                    cfg.model_save_root+'modelv62.pth')
            if iter == 7 and epoch % 2 ==0:
                ...
                if eval_psnr > best_psnr:
                    ...
                    torch.save({
                        ...
                        'optimizer': G_optimizer.state_dict(),
                        'best_psnr': best_psnr},
                        os.path.join(cfg.model_save_root, 'modelv62_best.pth'))
    ...
    with torch.no_grad():
        for iter, data in enumerate(test_loader):
            ...
            for tt in range(7):
                ...
                _,_,_,_,x = model(inputn, noise_map)
                x = torch.clamp(x,0,1)
