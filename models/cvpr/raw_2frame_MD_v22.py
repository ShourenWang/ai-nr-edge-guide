...
class SPADE(nn.Module):
    def __init__(self, x_nc, segmap_nc):
        super().__init__()
        self.meanblur = mean_blur(3, x_nc).cuda()
        self.get_edge_feature = nn.Conv2d(segmap_nc, x_nc, kernel_size=3, padding=1)
    ...
    def forward(self, x, segmap):
        ...
        x_mean = self.meanblur(x)
        e_mean = self.meanblur(edge)
        cov = self.meanblur(edge * x) - e_mean * x_mean
        var = self.meanblur(edge * edge) - edge * edge
        a = cov * self.div(var)
        b = x_mean - a * x_mean
        out = a * x + b
        return out
