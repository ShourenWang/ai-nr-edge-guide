...
class mean_blur(nn.Module):
    def __init__(self, ksize,outchannel):
        super(mean_blur, self).__init__()
        self.ksize = ksize
        self.out_channels = outchannel
        kernel2 = np.ones((self.ksize,self.ksize))/self.ksize/self.ksize
        kernel2 = torch.FloatTensor(kernel2).expand(outchannel, 1, self.ksize,self.ksize)
        self.weight2 = torch.nn.Parameter(data=kernel2, requires_grad=False)
...
class SPADE(nn.Module):
    def __init__(self, x_nc, segmap_nc):
        ...
        self.get_edge_feature = self.div = nn.Sequential(
            nn.Conv2d(segmap_nc, x_nc, kernel_size=3, padding=1),
            nn.ReLU(True),
            nn.Conv2d(x_nc, x_nc, kernel_size=3, padding=1),
            nn.ReLU6(True)
        )
    ...
    def forward(self, x, segmap):
        ...
        edge = self.get_edge_feature(segmap)
        edge = edge / 6.0
        out = x*edge + (1-edge)*self.convF(x)
        return out
...
class DownBlock_sade(nn.Module):
    ''' Downscale + (Conv2d => BN => ReLU)*2 '''
    def __init__(self, in_ch, out_ch, filter_lrelu_flag=['']):
        super(DownBlock_sade, self).__init__()
        self.spade = SPADE(in_ch, 1)
        self.convblock = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=1, stride=2),
            nn.LeakyReLU(0.2, inplace=False),
            CvBlock(out_ch, out_ch, filter_lrelu_flag)
...
class FastDVDnet(nn.Module):
    ...
    def __init__(self, num_input_frames=2):
        ...
        self.gm = nn.AdaptiveAvgPool2d((1,1))
    ...
    def forward(self, x, noise_map): #
        ...
        coff_a = noise_map[:, 0, :, :].unsqueeze(1)  # torch.sqrt(
        coff_b = noise_map[:, 1, :, :].unsqueeze(1)
        mean_x1 = self.gm(x1[:, 1, :, :]).unsqueeze(1)
        sigma_2 = torch.sqrt( coff_a * mean_x1 + coff_b )
        coff_b =  torch.sqrt(coff_b)
        # x_gray = torch.mean(x1, 1).unsqueeze(1)
        # x_edge = x_gray - self.blur1(x_gray)

        # First stage -- course reconstruction
        x_pool = self.temp2(self.pool(x0), self.pool(x1), self.pool(coff_b))
        x_pool_gray = torch.mean(x_pool,1).unsqueeze(1)

        edge = x_pool_gray - self.blur1(x_pool_gray)
        x_pool_upx2 = self.up(x_pool)
        x_course = self.temp1(x0, x1, coff_b, x_pool,  edge) + x_pool_upx2

        x_course_gray = torch.mean(x_course, 1).unsqueeze(1)
        edge = x_course_gray - self.blur1(x_course_gray)
        # edge = 0.5*edge + 0.5*x_edge
