...
class FreTransfer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        ll = torch.ones([B, 4, int(H / 2), int(W / 2)] ).cuda()
        hl = torch.ones([B, 4, int(H / 2), int(W / 2)] ).cuda()
        lh = torch.ones([B, 4, int(H / 2), int(W / 2)] ).cuda()
        hh = torch.ones([B, 4, int(H / 2), int(W / 2)] ).cuda()
...
class FreTransferInv(nn.Module):
    ...
    def forward(self, x):
        ...
        hhs = x[:, 12:16, :, :]
        B, C, H, W = lls.shape
        out = torch.ones([B, C, int(H * 2), int(W * 2)]).cuda()
        for i in range(C):
            ll = lls[:, i:i + 1, :, :]
            hl = hls[:, i:i + 1, :, :]
            lh = lhs[:, i:i + 1, :, :]
...
class Denoise_up(nn.Module):
    def __init__(self):
        ...
        self.get_edge_feature = nn.Sequential(
...
class MultiVideoDenoise(nn.Module):
    def __init__(self):
        ...
        self.blur1 = g_blur(3, 1, 1)

    def forward(self, ft0, ft1, gamma_up, denoise_down, coeff_a, coeff_b):
        ll0 = ft0[:, 0:4, :, :]
        ll1 = ft1[:, 0:4, :, :]

        edge = torch.mean(ll1,1).unsqueeze(1)
        edge = edge - self.blur1(edge)
        ...
        denoise_out = self.denoise(denoise_in,edge)
...
class MainDenoise(nn.Module):
    ...
    def forward(self, x, noise_map):
        ft0 = x[:, 0:4, :, :]  # 1*4*512*512, the t-1 fusion frame
        ft1 = x[:, 4:8, :, :]  # 1*4*512*512, the t frame
        coeff_a = noise_map[0,0,2,2]
        coeff_b = noise_map[0,1,2,2]
        ...
        gamma, denoise_out = self.vd(ft0_d2, ft1_d2, coeff_a, coeff_b) # 64,64
        denoise_out_d2 = self.fti(denoise_out) #128 128
        ...
        denoise_up_d1 = self.fti(denoise_out) #256
