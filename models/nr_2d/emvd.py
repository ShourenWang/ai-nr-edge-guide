...
class FreTransfer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        ll = torch.ones([B, 4, int(H / 2), int(W / 2)] )
        hl = torch.ones([B, 4, int(H / 2), int(W / 2)] )
        lh = torch.ones([B, 4, int(H / 2), int(W / 2)] )
        hh = torch.ones([B, 4, int(H / 2), int(W / 2)] )
...
class FreTransferInv(nn.Module):
    ...
    def forward(self, x):
        ...
        hhs = x[:, 12:16, :, :]
        B, C, H, W = lls.shape
        out = torch.ones([B, C, int(H * 2), int(W * 2)])
        for i in range(C):
            ll = lls[:, i:i + 1, :, :]
            hl = hls[:, i:i + 1, :, :]
            lh = lhs[:, i:i + 1, :, :]
